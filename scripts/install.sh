...
# Categories may be nested; a SKILL.md inside an existing skill directory is ignored.
declare -A WANT_DIRS
...
while IFS= read -r -d '' skill_file; do
  ...
  while [ "$parent_dir" != "$SOURCE_DIR" ]; do
...
done < <(find "$SOURCE_DIR" -mindepth 3 -name SKILL.md -print0 | sort -z)
