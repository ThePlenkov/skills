...
| Stop iterating after one pass / declare done because "threads look resolved" | Keep looping until the [exit gate](../SKILL.md#exit-gate--hard-stop-conditions) passes: `open_threads=0` AND CI green on HEAD AND no new bot comments AND no cycle-guard signal (reopened thread, same rule flagged 2+ times, empty /act loop). Stopping early is a violation. |
