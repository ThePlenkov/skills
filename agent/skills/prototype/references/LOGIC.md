...
2. **Read one keystroke (or one line)** at a time, dispatch to the **pure logic module** which returns the next state; the TUI replaces its current state with the returned next state (no in-place mutation by the TUI).
