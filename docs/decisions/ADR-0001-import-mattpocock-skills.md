...
| `codebase-design` | Vocab (module, depth, seam, leverage, locality) is excellent but the existing `engineering/api-and-interface-design` covers the surface. Borrow vocab into a shared reference doc, not a new skill. |
...
- `docs/graphs/generated/*` regenerated by `npm run graph:update` (85 skills, 163 edges — the +2 over the initial 161 came from `handoff → code-review-and-quality` and `skillmaker → writing-great-skills`, both added in subsequent `/act` review feedback commits).
