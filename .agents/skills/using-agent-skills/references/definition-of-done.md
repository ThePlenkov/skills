...
A standing, project-wide bar that every change must clear before it counts as done. Unlike acceptance criteria, which vary per task and answer "did we build the right thing?", the Definition of Done is the same every time and answers "is this finished to our standard?". Use it as the final gate in $skill{shared-plan} and any shipping workflow.
...
- [ ] Observability in place for new critical paths (logs, metrics, traces) (see $skill{debugging})
...
- **Per release**: the full checklist is the floor; add deploy-specific gates on top.
