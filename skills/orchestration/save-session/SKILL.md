...
## PR Handoff (merged from pr-handoff)
...
### When NOT to Use PR Handoff

- Single-shot tasks with no follow-up subagent (use save-session above).
...
- $skill{shared-plan} — for the broader project planning that this skill feeds into.
- $skill{act} — for the post-merge /act workflow (which runs after the handoff, on the just-opened PR).
- $skill{retrospect} — for the pattern of capturing what was learned.
- $skill{subagent-capsule} — for the structure of a PATCHER subagent's context capsule.
