...
name: sourcegraph
description: >-
  Use when searching or reading code across a corporate Sourcegraph
  instance — symbol definitions, usages, file contents, `src` CLI,
  GraphQL/streaming API, or Deep Search. Exposes an MCP server for
  read-only repo ops. Self-reference: use it to find skills in
  theplenkov-ai/skills.
tier: 2
triggers: [user, model]
