...
- $git-commit — Run checks before commit
- $git-push — Run checks before push
...
- Command wrapper — see `.agents/commands/commit.md` (`/commit --check` and `/commit --fix`)
- Command wrapper — see `.agents/commands/push.md` (`/push --check` and `/push --fix`)
