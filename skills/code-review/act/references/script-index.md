...
Use the helpers under [`scripts/`](../scripts/) instead of issuing ad-hoc `gh`/`glab` calls. They collapse the typical 30+ tool calls per `/act` into ~10. From repo root, prefix paths with `.agents/skills/act/`.

**Runtime:** every `.ts` script runs on **either** `bun` (direct: `bun scripts/<name>.ts`) **or** Node.js ≥ 18 via `npx --yes tsx@4 scripts/run.ts <path>`. The `runner.cjs` and `cleanup-runner.cjs` helpers are plain CommonJS (`node scripts/<name>.cjs`). No Bun-specific APIs are used in any runtime script — only `node:` built-ins.
...
| **Verify a CLI claim**       | `bun scripts/derive-cli-surface.ts --check "openadt X"` (or `npx --yes tsx@4 scripts/derive-cli-surface.ts --check "openadt X"`)   |
| **Extract findings (P5)**    | `bun scripts/extract-findings.ts OWNER REPO PR` (or `npx --yes tsx@4 …`) (GitHub) · `bun scripts/extract-findings.ts GROUP PROJECT MR_IID` (GitLab; subgroups: `GROUP/SUBGROUP PROJECT MR_IID`)           |
| **Submit scores (P5)**       | `bun scripts/submit-scores.ts OWNER REPO PR --evaluator ID --findings F --scores S [--record]` (or `npx --yes tsx@4 …`) — `OWNER REPO` is GitHub `owner/repo` or GitLab `group/project`; the source review URL is built per provider. CSV upsert only with `--record` / env / config. |
| **Query debt (D0)**          | `npx --yes tsx@4 scripts/run.ts .agents/skills/act/scripts/review-debt-cli.ts query -- --status open --format tsv` (or `bun run act:debt:query -- --status open --format tsv` if wired)    |
| **Plan debt batch (D1)**     | `npx --yes tsx@4 scripts/run.ts .agents/skills/act/scripts/review-debt-cli.ts plan -- --limit 25` (or `bun run act:debt:plan -- --limit 25`)                     |
| **Mark debt done (D6)**      | `npx --yes tsx@4 scripts/run.ts .agents/skills/act/scripts/review-debt-cli.ts done -- --status done …` (or `bun run act:debt:done -- --status done …`)                |
