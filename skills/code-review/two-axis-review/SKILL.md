...
1. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.) — fetch via the appropriate integration skill (`$skill{external-tools}`, `$skill{external-tools}`, etc.).
...
- `engineering/api-and-interface-design`, `engineering/frontend-ui-engineering`, `engineering/performance-optimization`, `engineering/security-and-hardening` — module-shape and cross-cutting concerns.
