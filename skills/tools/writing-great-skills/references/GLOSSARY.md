...
The terms below are grouped by axis (Invocation, Information Hierarchy, Steering, Pruning) and tag (_failure mode_ for the diagnosis-and-cure pairs). The bold terms in each **definition** body are themselves defined as headings in this glossary; find them by their heading. Axis names and the _failure mode_ tag are grouping markers, not terms — they are not looked up separately.
...
A skill that the agent can see and fire autonomously — set by the frontmatter `disable-model-invocation: false` (the default; only an explicit `true` makes the skill user-invoked). The skill still keeps its `description:` field (every skill does), and the agent reads it on every turn, so model-invocation pays a permanent **context load** in exchange for that discoverability. The human can still type the skill's name, so model-invocation always _includes_ user reach — there is no model-only state. Reachable by other skills: the description that makes it agent-discoverable also makes it invocable from another skill. A model-invoked skill whose content is all **reference** is one home for shared reference (another skill can invoke it, so reference needed by several skills lives in one place). Pick model-invocation only when the agent must reach the skill on its own; if it never fires except by hand, set `disable-model-invocation: true` and pay no context load.

_Avoid_: ability, tool, capability

### User-Invoked

A skill that hides itself from the agent's reach while staying available to the human who types its name. The mechanism in this repo is `disable-model-invocation: true` in the frontmatter, which strips the skill from the agent's reach while leaving the human-facing `description` field (and any other agent-facing metadata) intact for the agent's index to read. The user-invoked skill is user-_only_, where **model-invoked** is user-_and-agent_; the trade is **cognitive load** on the human (who must remember the name) for zero **context load** on the agent (no description is loaded into the agent's window). No other skill can fire a user-invoked skill by name.

**Scope note.** The `disable-model-invocation: true` mechanism is honoured by the direct skill installation paths this repository documents (`npx skills add <owner>/<repo> --skill <name>`, the local `.agents/skills/` symlink view, and the `theplenkov-ai/skills` marketplace). It is **not** universally honoured by every consumer — Claude Code plugin installations and some third-party runners ignore the field and treat every installed skill as model-invocable, which means a user-invoked skill in this repo can still be reached by the agent when installed via those paths. If the user-invocation guarantee matters, verify the install path before counting on the zero-context-load trade.

_Avoid_: procedure, workflow, command

### Description

The skill's machine-readable trigger, and the one **context pointer** a **model-invoked** skill is forced to keep loaded at all times. The model-facing description carries the trigger list (when to fire); the human-facing description (for user-invoked skills) carries only a one-line summary. Whether the description reaches the agent is controlled by the frontmatter `disable-model-invocation: true` field, **not** by the presence or absence of the description itself — every skill in this repo keeps a `description:` field, and the user's invocation mode is the only thing that varies. A model-invoked skill's primary **context load** cost.
...
The cost a **model-invoked** skill imposes on the agent's context window — its **description** sits in the window every turn, spending both tokens and attention. What **user-invoked** skills escape when `disable-model-invocation: true` makes the description unreachable by the agent, and the brake on splitting into more model-invoked skills.
...
A **user-invoked** skill whose job is to point at your other user-invoked skills — naming each and when to reach for it — so the human has one skill to remember instead of many. It can only hint, never fire them: user-invoked skills are hidden from the agent's reach by `disable-model-invocation: true`, so no other skill (including the router) can reach them. The cure for **cognitive load** when user-invoked skills multiply.

_Avoid_: dispatcher, menu, registry, index, router procedure

### Granularity

How finely you divide skills. Finer division spends one of the two loads: more **model-invoked** skills spend **context load** (more descriptions crowding the window and competing for attention); more **user-invoked** skills spend **cognitive load** (more for the human to remember and reach for). Two cuts guide the division. By **invocation**, split off a model-invoked skill where you have a distinct **leading word** to trigger it — a trigger word you actually use in your prompts. By **sequence**, split a run of **steps** where a step's **post-completion steps** need hiding, since isolating it in its own context clears what follows. Beware the reverse: merging sequences exposes each step's post-completion steps to what follows, inviting premature completion. (Note: the user-invoked / model-invoked split is controlled by `disable-model-invocation: true`, not by the presence or absence of the `description:` field. Splitting the skill body always keeps the frontmatter.)
