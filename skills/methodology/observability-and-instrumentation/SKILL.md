...
### NOT for:
- Diagnosing a failure happening right now — use the $skill{debugging} skill (observability is what makes that skill fast next time)
- Profiling and optimizing measured slowness — use the $skill{performance-investigation} skill
- Launch-day monitoring checklists and rollback triggers — see the $skill{shipping-and-launch} skill; this skill covers the instrumentation that feeds them
...
**Never log secrets, tokens, passwords, or full PII.** This is a hard rule from the $skill{security-and-hardening} skill — telemetry pipelines are a classic data-leak path. Allowlist fields; don't log whole request bodies.
