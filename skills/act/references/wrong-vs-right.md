...
| Create a `review/<name>` base branch in the same repo to review already-merged `main` commits | Don't. Use a fork (`[shadow-fork](shadow-fork/SKILL.md)`), run review tools directly on `main`, or use an ephemeral empty branch you delete immediately. Custom base branches go stale and GitHub auto-creates reverse PRs on merge. See [footguns.md](footguns.md#review-only-prs). |
